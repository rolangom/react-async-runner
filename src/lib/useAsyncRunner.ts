...
const normalizeModule = (module: string) => {
  if (remoteRegexp.test(module) || localFileRegexp.test(module)) return module;
...
export const defaultImportsRevolver = (
...
interface IUseAsyncRunnerReturn {
  isLoading: boolean;
...
export type ImportResolvers = typeof defaultImportsRevolver;
export const useAsyncRunner = ({
  files,
  scope,
  disableCache,
  resolveImports = defaultImportsRevolver,
}: UseAsyncRunnerProps): IUseAsyncRunnerReturn => {
